...
### July 2026
- **GigaPath-Flash and GigaTIME-Flash**: We are releasing lightweight "Flash" variants of GigaPath and GigaTIME for faster, more memory-efficient inference and finetuning. See the [GigaPath-Flash model](https://huggingface.co/prov-gigapath/prov-gigapath-flash) and [GigaTIME-Flash model](https://huggingface.co/prov-gigatime/gigatime-flash)

### December 2025
- **GigaTIME**: We released GigaTIME, scaling tumor microenvironment modeling using a virtual population generated by multimodal AI. Check out the [repo](https://github.com/prov-gigatime/GigaTIME), [paper](https://www.cell.com/cell/fulltext/S0092-8674%2825%2901312-1), and [blog](https://www.microsoft.com/en-us/research/blog/gigatime-scaling-tumor-microenvironment-modeling-using-virtual-population-generated-by-multimodal-ai/).
...
GigaPath-Flash is a lightweight variant of the GigaPath slide encoder for faster,
more memory-efficient slide-level inference and finetuning. It uses a LongNet
backbone with 12 layers and a 384-dim hidden size (~21M parameters), paired with a
384-dim DINOv2-small tile encoder (ViT-S/16, ~22M parameters). Compared to the
original GigaPath slide encoder (12L/768d over 1536-dim tile embeddings), both the
slide encoder hidden dim and the input tile-embedding dim are 384.

Model architecture names: `gigapath_slide_enc12l384d` (slide encoder,
`input_dim=384`, `latent_dim=384`) and `gigapath_tile_enc_dinov2s` (tile encoder).
...
Raw pretraining checkpoints contain parameters not needed for downstream use.
Strip them and convert to the release formats:
...
Both mirror the format of the original Prov-GigaPath `slide_encoder.pth` and tile
encoder.
...
Point the finetune scripts at the converted slide-encoder checkpoint via
`GIGAPATH_FLASH_CKPT`:
